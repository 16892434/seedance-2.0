...
| Atlas Cloud | third-party aggregator surface | OpenAI-compatible API reselling hosted Seedance 2.0 alongside other models | Verified 2026-06-13: Atlas Cloud publishes live Seedance 2.0 endpoints (text-to-video, image-to-video, reference-to-video, plus fast variants) through a unified OpenAI-compatible API. One of several aggregators that resell Seedance access; treat pricing, model IDs, quotas, and guardrail behavior as aggregator-specific, recheck before use, and never present them as official ByteDance behavior. The repo endorses no reseller; listed for completeness alongside other surfaces. |
